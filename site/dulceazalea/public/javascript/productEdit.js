...
window.addEventListener('load', function () {
  ...
  campoImagen.addEventListener('blur', function () {
    ...
    if (!hasExtension('image', ['.jpg', '.jpeg', '.png'])) {
      errorImage.innerHTML =
        'El formato de la imágen es incorrecto. Por favor, verifique que el formato sea jpg, jpeg o png';
  ...
  campoImagen.addEventListener('change', function (e) {
    ...
    reader.onload = function () {
      vistaPreviaEdit.src = reader.result;
    };
  });
    ...
    switch (true) {
      case this.value.length < 7 || this.value.length > 120:
        errorDescription.innerHTML =
          'La descripción debe tener entre 7 y 120 caracteres';
        this.classList.add('is-invalid');
        break;
      default:
        this.classList.remove('is-invalid');
